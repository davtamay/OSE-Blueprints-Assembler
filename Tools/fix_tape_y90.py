...
# 1. Flip winding order (swap v1,v2 in every triangle) to fix normals
...
# 2. Negate all normals (to match flipped winding)
for mesh in gltf.meshes:
    for prim in mesh.primitives:
...
# 3. Double-sided material
for mat in gltf.materials:
    mat.doubleSided = True
    print(f"Material '{mat.name}' -> doubleSided")

# 4. Bake ONLY 90 deg Y rotation (what the user confirmed works in Unity)
# glTF quaternion format: [x, y, z, w]
angle = math.radians(90)
qy = [0, math.sin(angle/2), 0, math.cos(angle/2)]
root = gltf.nodes[gltf.scenes[gltf.scene].nodes[0]]
root.rotation = qy
print(f"Set Y=90 rotation: {qy}")
