...
**Audit existing writers before adding a parallel one.** When the user reports a visibility / scale / position / color bug on an existing object, FIRST grep every system that writes to that property (`transform.localScale`, `material.color`, `_BaseColor`, `MeshRenderer.enabled`, `MaterialPropertyBlock`, etc.) before designing a new component to "fix" it. Adding a parallel writer creates a per-frame fight where one silently wins, and you'll spend the next session debugging the wrong code. **`MaterialPropertyBlock` writes silently override `sharedMaterial.SetColor` writes for any property in the block** — if a runtime value isn't taking effect, look for a PropertyBlock writer first. When the user says "I think there's another system at play", treat it as primary diagnostic data, not a hunch — they almost always know their domain better than your clean-looking code does.
