 ...
 * @name Pulley
 * @category Printed
 * @using 1 x m2_5 nut, normal or nyloc
 * @using 1 x m2_5x10 set screw or 1 x m2_5x8 grub screw
 */
...
m2_5_dia = 2.7;		// 3mm hole diameter
m2_5_nut_hex = 1;		// 1 for hex, 0 for square nut
m2_5_nut_flats = 5.2;	// normal M2_5 hex nut exact width = 5.5
m2_5_nut_depth = 2.2;	// normal M2_5 hex nut exact depth = 2.4, nyloc = 4
...
m2_5_nut_points = 2*((m2_5_nut_flats/2)/cos(30)); // This is needed for the nut trap
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	difference()
	 {	 
		...
		if ( pulley_b_ht < m2_5_nut_flats ) { echo ("CAN'T DRAW CAPTIVE NUTS, HEIGHT LESS THAN NUT DIAMETER!!!"); } else {
		if ( (pulley_b_dia - motor_shaft)/2 < m2_5_nut_depth + 3 ) { echo ("CAN'T DRAW CAPTIVE NUTS, DIAMETER TOO SMALL FOR NUT DEPTH!!!"); } else {
			...
			{
				//entrance
				translate([0,-pulley_b_ht/4-0.5,motor_shaft/2+m2_5_nut_depth/2+nut_shaft_distance]) cube([m2_5_nut_flats,pulley_b_ht/2+1,m2_5_nut_depth],center=true);
	
				//nut
				if ( m2_5_nut_hex > 0 )
					{
						// hex nut
						translate([0,0.25,motor_shaft/2+m2_5_nut_depth/2+nut_shaft_distance]) rotate([0,0,30]) cylinder(r=m2_5_nut_points/2,h=m2_5_nut_depth,center=true,$fn=6);
					} else {
						// square nut
						translate([0,0.25,motor_shaft/2+m2_5_nut_depth/2+nut_shaft_distance]) cube([m2_5_nut_flats,m2_5_nut_flats,m2_5_nut_depth],center=true);
					}
	
				//grub screw hole
				rotate([0,0,22.5])cylinder(r=m2_5_dia/2,h=pulley_b_dia/2+1,$fn=8);
			}
		}}
	 }
