...
namespace OSE.Content.Loading
{
    ...
    public static class MachinePackageNormalizer
    {
        public static void Normalize(MachinePackageDefinition package)
        {
            ...
            // G.2.5.b: synthesize stepPose entries from TaskOrderEntry.endTransform
            // so every runtime consumer of FindPartStepPose automatically sees
            // task-owned inline poses. Must run BEFORE BakePoseTable so the
            // synthesized entries flow into the resolver index.
        ...
        /// <see cref="BakeTaskEndTransforms"/>: one entry per task whose
        /// <see cref="TaskOrderEntry.endTransform"/> is authored. The synth entries flow
        /// through <see cref="PreviewConfigLookup.FindPartStepPose"/> without any
        /// consumer code change (G.2.5.b bridge).
        ///
        /// <para>The save-path filter in <c>TTAW.WriteJson.cs</c> and the pose-resolver
        /// index in <see cref="PoseResolverIndex"/> recognise this prefix so synthesized
        /// entries never round-trip to JSON — they are regenerated on every load from
        /// the authored <c>TaskOrderEntry.endTransform</c>.</para>
        /// </summary>
        public const string TaskEndBakeLabel = "__task_end_bake";

        /// <summary>
        /// G.2.5.b normalization pass. For every step, walks
        /// <see cref="StepDefinition.taskOrder"/> in order and, for each
        /// <see cref="TaskOrderEntry"/> with an authored <c>endTransform</c>,
        /// synthesizes a <see cref="StepPoseEntry"/> on the
        /// <see cref="PartPreviewPlacement"/> of the part that task touches.
        ///
        /// <para>Ownership rule for (partId P, stepId S): the LAST task in the
        /// step's taskOrder touching P whose endTransform is non-null owns the
        /// step's end-pose for P. Subsequent synthesizer passes overwrite
        /// earlier entries, matching the runtime "last task wins" semantics.</para>
        ///
        /// <para>Task→part mapping:
        /// <list type="bullet">
        ///   <item>Part task (kind = "part"): partId = <c>TaskInstanceId.ToPartId(entry.id)</c></item>
        ///   <item>Tool task (kind = "toolAction"): partId = the associated target's <c>associatedPartId</c></item>
        /// </list>
        /// All other kinds (wire / confirm) contribute nothing.</para>
        ///
        /// <para>Synthesized entries are labelled with
        /// <see cref="TaskEndBakeLabel"/> so the save path (<c>TTAW.WriteJson.cs</c>)
        /// and the <see cref="PoseResolverIndex"/> filter them out — the source of
        /// truth remains <see cref="TaskOrderEntry.endTransform"/> on the task,
        /// not the synthesized placeholder.</para>
        /// </summary>
        private static void BakeTaskEndTransforms(MachinePackageDefinition package)
        {
            ...
            // Index placements for fast insertion.
            ...
            // Index requiredToolActions by id (action.id is unique within a step).
            ...
            // Index targets by id (for associatedPartId resolution).
            ...
            foreach (var step in package.steps)
            {
                ...
                foreach (var entry in step.taskOrder)
                {
                    ...
                    // Replace any earlier entry for this stepId (last task's endTransform
                    // wins — matches runtime "final pose after step completes" semantics).
