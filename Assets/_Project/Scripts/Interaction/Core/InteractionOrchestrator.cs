...
namespace OSE.Interaction
{
    ...
    public sealed class InteractionOrchestrator : MonoBehaviour
    {
        ...
        private ToolActionCoordinator _toolAction;
        ...
        private void Bootstrap(InteractionMode mode)
        {
            ...
            _cameraRouter = new CameraIntentRouter(
                ...
                isToolModeLocked: ToolActionCoordinator.IsToolModeLockedForParts,
            ...
            _toolAction = new ToolActionCoordinator(
                _settings, _previewController, _toolPreview, _partBridge, _actionBridge,
                _cameraRig, _guidanceService, _targetSphereAnimator, _persistentToolController,
        ...
        {
            // Re-subscribe after disable/enable cycles. Bootstrap sets _bootstrapped=true
            // then calls SubscribeSessionEvents() directly on first run; subsequent
            // enable cycles use this path.
            if (_bootstrapped)
                SubscribeSessionEvents();
        }
        ...
        private void Update()
        {
            ...
            if (ToolActionCoordinator.IsToolModeLockedForParts() && CurrentState == InteractionState.DraggingPart)
            {
                _actionBridge?.OnPartReleased();
        ...
        private void UpdateHover()
        {
            ...
            if (ToolActionCoordinator.IsToolModeLockedForParts())
            {
                HoveredPart = null;
                return;
            }
        ...
        private void HandleSelect(InteractionIntent intent)
        {
            ...
            bool toolLocked = ToolActionCoordinator.IsToolModeLockedForParts();
            OseLog.VerboseInfo($"[Interaction] HandleSelect: toolLocked={toolLocked}, hitTarget={intent.HitTarget?.name ?? "null"}");

            if (toolLocked)
            {
                _toolAction.RouteToolAction(intent.ScreenPosition);
                return;
            }
