    ...
    public sealed class PackagePartSpawner : MonoBehaviour, Interaction.ISpawnerQueryService, IStepAwarePositioner
    {
        ...
        private async Task SpawnGlbPartsAsync(CancellationToken ct)
        {
            ...
            for (int i = 0; i < _spawnedParts.Count; i++)
            {
                ...
                if (Application.isPlaying)
                {
                    // GLBs don't have colliders by default — add mesh colliders so the
                    // part is clickable in scene view and detectable by XRI rays.
                    // (Primitives get EnsureColliders in the sync path; GLBs need it here.)
                    EnsureColliders(loaded);
