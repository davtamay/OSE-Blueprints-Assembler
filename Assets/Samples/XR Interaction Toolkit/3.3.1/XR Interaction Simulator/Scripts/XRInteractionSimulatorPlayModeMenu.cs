...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.InteractionSimulator
{
    class XRInteractionSimulatorPlayModeMenu : MonoBehaviour
    {
        ...
        {
            bool hasConfiguredHandVisualizers = HasConfiguredHandVisualizers();
            bool hasSceneHandVisualizers =
                Object.FindObjectsByType<XRHandMeshController>(FindObjectsInactive.Include, FindObjectsSortMode.None).Length > 0 ||
                Object.FindObjectsByType<XRHandSkeletonDriver>(FindObjectsInactive.Include, FindObjectsSortMode.None).Length > 0 ||
                Object.FindObjectsByType<XRHandTrackingEvents>(FindObjectsInactive.Include, FindObjectsSortMode.None).Length > 0;

            bool showWarning = !(hasConfiguredHandVisualizers || hasSceneHandVisualizers);
            m_InputMenuHandVisualizerWarningPanel.SetActive(showWarning);
            m_HandMenuHandVisualizerWarningPanel.SetActive(showWarning);
        }

        static bool HasConfiguredHandVisualizers()
        {
            var modalityManagers = Object.FindObjectsByType<XRInputModalityManager>(FindObjectsInactive.Include, FindObjectsSortMode.None);
            for (var index = 0; index < modalityManagers.Length; ++index)
            {
                var modalityManager = modalityManagers[index];
                if (modalityManager == null)
                    continue;

                if (HasHandVisualizerComponents(modalityManager.leftHand) || HasHandVisualizerComponents(modalityManager.rightHand))
                    return true;
            }
        ...
        static bool HasHandVisualizerComponents(GameObject handRoot)
        {
            if (handRoot == null)
                return false;

            return handRoot.GetComponentInChildren<XRHandMeshController>(true) != null ||
                   handRoot.GetComponentInChildren<XRHandSkeletonDriver>(true) != null ||
                   handRoot.GetComponentInChildren<XRHandTrackingEvents>(true) != null;
        }
#endif
