...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.Hands
{
    ...
    public class ReleaseThresholdButtonReader : MonoBehaviour, IXRInputButtonReader
    {
        ...
        void Update()
        {
            ...
            var sourcePerformed = m_ValueInput.ReadIsPerformed();
            if (sourcePerformed)
            {
                // Some simulated hand inputs report a performed bool without driving a stable analog value.
                // Treat performed as authoritative so hand select/grab can still engage.
                m_IsPerformed = true;
            }
            else
            {
                m_IsPerformed = prevPerformed
                    ? pressAmount > m_ReleaseThreshold
                    : pressAmount >= m_PressThreshold;
            }
